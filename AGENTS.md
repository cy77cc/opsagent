...
- Viper（配置管理）
- zerolog（结构化日志）
- prometheus/client_golang（Prometheus 指标导出）
- lumberjack（日志轮转）
- fsnotify（文件系统监听）
- testify（测试断言）
- net/http（服务与客户端）
...
- `internal/health`：健康检查接口定义（Statuser）
- `internal/sandbox`：nsjail 沙箱执行引擎、seccomp 策略、审计日志
...
```bash
go test ./...                       # 基础测试
make test-race                      # 含 race detector
make test-cover                     # 覆盖率报告（80% 门槛）
